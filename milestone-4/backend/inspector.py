    ...
    for y in range(0, h - patch_size + 1, stride):
        for x in range(0, w - patch_size + 1, stride):
            ...
            if conf >= confidence_thresh:
                ...
                cv2.putText(annotated, f"{label} {int(conf)}%", (x, max(y-5, 15)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
                defects.append({"type": label, "confidence": round(conf,2), "bbox":[x,y,x+patch_size,y+patch_size]})
